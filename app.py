...
def draw_boxes(img_bgr, boxes_xyxy, labels, scores, id2name, color=(0, 255, 0)):
    out = img_bgr.copy()
...
def match_report(pred_boxes, pred_catids, gt_boxes, gt_catids, iou_thr=0.25):
...
def build_cascade_model(num_classes, min_size=600, max_size=1000, cascade_cfg=None):
...
def parse_checkpoint_and_build_model(ckpt_path, coco):
    ...
    if isinstance(ckpt, dict) and ("model_state_dict" in ckpt or "model_state" in ckpt):
        ...
        contig_to_catid = normalize_int_dict(ckpt.get("contig_to_catid", None))
...
class CascadeFRCNNGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Cascade FRCNN Desktop App")
        ...
        self.btn_run = QPushButton("RUN Predict All")
        ...
        self.btn_save = QPushButton("Save fail cases to fail_cases/")
        self.btn_cm = QPushButton("Show Confusion Matrix")
        ...
        self.lbl_summary = QLabel("SUMMARY: (run to compute)")
        self.lbl_summary.setWordWrap(True)

        self.lbl_info = QLabel("Select checkpoint, folder, GT json, then RUN.")
        ...
        self.btn_imgs.clicked.connect(self.pick_folder)
        self.btn_gt.clicked.connect(self.pick_gt)
        self.btn_run.clicked.connect(self.run_predict_all)
        self.btn_prev.clicked.connect(self.prev_img)
        self.btn_next.clicked.connect(self.next_img)
        self.chk_wrong.stateChanged.connect(self.toggle_wrong_only)
    ...
    def load_everything(self):
        if not self.model_path or not Path(self.model_path).is_file():
            self.msg("Please choose model checkpoint (.pt)", "Error")
            return False

        if not self.img_dir or not Path(self.img_dir).is_dir():
            self.msg("Please choose Image Folder", "Error")
            return False

        if not self.gt_json or not Path(self.gt_json).is_file():
            self.msg("Please choose GT COCO json", "Error")
            return False

        self.coco = COCO(self.gt_json)
        self.coco = fix_coco_gt(self.coco)

        cats = self.coco.loadCats(self.coco.getCatIds())
        cats_sorted = sorted(cats, key=lambda c: c["id"])
        self.catid_to_name = {c["id"]: c["name"] for c in cats_sorted}

        self.model, self.contig_to_catid, _, self.min_size, self.max_size = parse_checkpoint_and_build_model(
            self.model_path,
            self.coco
        )
    ...
    def predict_for_bgr(self, img_bgr):
        rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
    ...
    def build_class_detail_text(self, rec):
    ...
    def run_predict_all(self):
        ...
        for i, im in enumerate(images, start=1):
            ...
            pb, ps, pc = self.predict_for_bgr(img)
            gb, gc = self.gt_for_fname(fname)

            tp, fp, fn, matches = match_report(pb, pc, gb, gc, iou_thr=IOU_TH)
            wrong = (fp > 0) or (fn > 0)
    ...
    def refresh_view(self):
        ...
        qp = QPixmap.fromImage(qimage_from_bgr(vis_pred)).scaled(
            self.lbl_pred.width(),
            self.lbl_pred.height(),
            Qt.KeepAspectRatio,
            Qt.SmoothTransformation
        )
        qg = QPixmap.fromImage(qimage_from_bgr(vis_gt)).scaled(
            self.lbl_gt.width(),
            self.lbl_gt.height(),
            Qt.KeepAspectRatio,
            Qt.SmoothTransformation
        ...
        class_detail = self.build_class_detail_text(rec)
    ...
    def export_csv(self):
        if not self.results:
            self.msg("Run prediction first.", "Error")
            return
        ...
        for fname, rec in self.results.items():
            ...
            rows.append({
                ...
                "detail": self.build_class_detail_text(rec).replace("\n", " || ")
            })

        pd.DataFrame(rows).to_csv(out_path, index=False)
        self.msg(f"Saved: {out_path}", "CSV")
    ...
    def save_fail_cases(self):
        ...
        self.msg(f"Saved {saved} images to: {out_dir}", "Done")
    ...
    def show_confusion_matrix(self):
        ...
        dlg = ImageDialog("Confusion Matrix", pix, parent=self)
        dlg.exec_()
